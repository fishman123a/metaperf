...
mc_connect = np.concatenate([
    pd.read_csv(
        os.path.join(PROJECT_ROOT, "data/mc/connect/%d/computation/cpu.csv" % i))[
        'total'].to_numpy()
    for i in range(1, 11)]
)

mc_interaction = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/mc/battle/computation/cpu.csv"))[
    'total'].to_numpy()

mc_general = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/mc/stand_still_no_entity/computation/cpu.csv"))[
    'total'].to_numpy()

mc_android_connect = np.concatenate([
    pd.read_csv(
        os.path.join(PROJECT_ROOT, "data/mc_android/connect/%d/computation/cpu.csv" % i))[
        'total'].to_numpy()
    for i in range(1, 9)]
)

mc_android_interaction = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/mc_android/battle/computation/cpu.csv"))[
    'total'].to_numpy()

mc_android_general = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/mc_android/stand_still/computation/cpu.csv"))[
    'total'].to_numpy()

roblox_connect = np.concatenate([
    pd.read_csv(
        os.path.join(PROJECT_ROOT, "data/roblox/connect/%d/computation/cpu.csv" % i))[
        'total'].to_numpy()
    for i in range(1, 6)]
)

roblox_interaction = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/roblox/rub/computation/cpu.csv"))[
    'total'].to_numpy()

roblox_general = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/roblox/stand_still/computation/cpu.csv"))[
    'total'].to_numpy()

roblox_android_connect = np.concatenate([
    pd.read_csv(
        os.path.join(PROJECT_ROOT, "data/roblox_android/connect/%d/computation/cpu.csv" % i))[
        'total'].to_numpy()
    for i in range(1, 6)]
)

roblox_android_interaction = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/roblox_android/rub/computation/cpu.csv"))[
    'total'].to_numpy()

roblox_android_general = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/roblox_android/rub/computation/cpu.csv"))[
    'total'].to_numpy()

vrchat_connect = np.concatenate([
    pd.read_csv(
        os.path.join(PROJECT_ROOT, "data/vrchat/connection/%d/computation/cpu.csv" % i))[
        'total'].to_numpy()
    for i in range(2, 6)]
)

vrchat_interaction = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/vrchat/item_interaction/computation/cpu.csv"))[
    'total'].to_numpy()

vrchat_general = pd.read_csv(
    os.path.join(PROJECT_ROOT, "data/vrchat/no_model_loading/computation/cpu.csv"))[
    'total'].to_numpy()
...
def run(fig, ax, connection,
    ...
    for i in range(len(x)):
        x[i] = x[i] - 2 * width
    lgd = ax.legend(loc='upper right', fontsize=40, edgecolor="white")
...
if __name__ == '__main__':
    ...
    run(fig1, ax1, connection=get_mean_df([mc_connect, roblox_connect, vrchat_connect],
        ...
        general=get_mean_df([mc_general, roblox_general, vrchat_general],
                            ['MC', 'Roblox', 'VR Chat']), output_file="cpu_bar_pc.pdf"),
    # fig2, ax2 = plt.subplots(figsize=fs, dpi=dpi)
    # run(fig2, ax2, connection=get_mean_df([mc_android_connect, roblox_android_connect],
    #                                     ['MC', 'Roblox']),
    #     interaction=get_mean_df([mc_android_interaction, roblox_android_interaction],
    #                             ['MC', 'Roblox']),
    #     general=get_mean_df([mc_android_general, roblox_android_general],
    #                         ['MC', 'Roblox']), output_file="cpu_bar_android.pdf", y_lim=0.8)
